...
const REFERENCE_GRID_CELLS = [
  { row: 1, col: 1, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 5, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 10, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 15, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 20, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 25, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 30, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 35, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 40, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 45, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 50, color: "rgba(0, 96, 255, 0.68)" },
  { row: 1, col: 55, color: "rgba(0, 96, 255, 0.68)" },
  { row: 5, col: 45, color: "rgba(255, 0, 0, 0.76)" },
  { row: 5, col: 50, color: "rgba(255, 0, 0, 0.76)" },
  { row: 20, col: 15, color: "rgba(0, 96, 255, 0.68)" },
  { row: 20, col: 25, color: "rgba(0, 96, 255, 0.68)" },
  { row: 20, col: 35, color: "rgba(0, 96, 255, 0.68)" },
  { row: 20, col: 45, color: "rgba(0, 96, 255, 0.68)" },
];
...
mapImage.onerror = () => {
  console.error("The embedded map image could not be loaded.");
};
...
function pickRandomMapImage() {
  const embeddedSources = Array.isArray(window.MAP_IMAGE_SOURCES) ? window.MAP_IMAGE_SOURCES : null;
  const localSources = [
    "assets/imagemap_pigeon_1.png",
    "assets/imagemap_pigeon_2.png",
    "assets/imagemap_pigeon_3.png",
  ];
  const sources = embeddedSources?.length ? embeddedSources : localSources;
...
function normalizePigeonPlacement(placement, index) {
  ...
  const inventory = parseInventoryValue(inventoryOverride ?? placement.inventory ?? placement.Inventory);
  const productLink =
...
function prepareMapTexture(sourceImage) {
  ...
  drawReferenceGridCells(textureContext, textureWidth, mapY, mapHeight);
  drawGrid(textureContext, textureWidth, textureHeight, 0, 0, TOTAL_TEXTURE_ROWS);

  return textureCanvas;
}

function drawReferenceGridCells(context, width, mapY, mapHeight) {
  ...
  for (const cell of REFERENCE_GRID_CELLS) {
    const zeroBasedCol = cell.col - 1;
    const zeroBasedRow = cell.row - 1;
    if (
      zeroBasedCol < 0 ||
      zeroBasedCol >= GRID_COLUMNS ||
      zeroBasedRow < 0 ||
      zeroBasedRow >= GRID_ROWS
    ) {
      continue;
    }
    ...
    context.fillRect(
      zeroBasedCol * cellWidth,
      mapY + zeroBasedRow * cellHeight,
      cellWidth,
      cellHeight,
    );
  }
